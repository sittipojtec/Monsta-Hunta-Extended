...
void player::operator++() {
  ...
  cout << "Exp +4" << endl;
  exp+=3; //Defeat 3 monsters to win
...
int player::monster3(monster &mon) {
  BST b;
	srand((int)time(0));
  int choice, stop = 0, action, in, i, input;
  int c[7];
...
int player::monster4(monster &mon) {
  BST b;
	srand((int)time(0));
  int choice, stop = 0, action, in, i, input;
  int c[7];
  monster m("Tiamat", 100);
  cout << "=================================" << endl;
  cout<<"Binary tree has been created with this order:";
  for(i=1;i<6;i++){
    int q = rand()%7+9;
    //cout<<q<<endl;
    b.insert_node(q); 
  ...
  while (1){
    ...
    int a;
    cout<<endl;
    cout<<"In order to advance, you have to solve the Binary Search Tree in a preorder pattern. Enter each digit one by one."<<endl;
    a = b.checkTree();
    if (a == 0){
      cout<<"The elden tree has been solved! You have been blessed and feel stronger, move forward hero!"<<endl;
			atk = 20;
      break;
    }
    else{
  		hp-=5;
  		cout<<"You have incorrectly solved the puzzle. You take 5 Health Damage."<<endl;
  		cout<<"You're remaining hp is "<<hp<<endl;
      if (hp <= 0){
        die();
        return -1;
      }
